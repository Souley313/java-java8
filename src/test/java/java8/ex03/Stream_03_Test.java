...
public class Stream_03_Test {
	...
	public void test_joining() throws Exception {
		...
		// par le caractère "|"
		String result = customers.stream().sorted((client1, client2) -> client1.getFirstname().compareTo(client2.getFirstname()))
				.map(client -> client.getFirstname()).collect(Collectors.joining("|"));
	...
	public void test_grouping() throws Exception {
		...
		Map<Customer, List<Order>> result = orders.stream().collect(Collectors.groupingBy(co -> co.getCustomer()));
