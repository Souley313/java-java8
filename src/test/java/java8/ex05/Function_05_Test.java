...
public class Function_05_Test {
	...
	Consumer<Person> changePasswordToSecret = p -> p.getPassword().equals("secret");
	...
	public void test_consumer() throws Exception {
		...
		// en "secret"
		personList.forEach(p -> p.getPassword().equals("secret"));
		...
		/**
		 * for(Person p : personList) { verifyAge.accept(p); verifyPassword.accept(p); }
		 */
	}
}
